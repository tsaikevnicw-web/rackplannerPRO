...
const NetworkTopologyLight = ({ nsSpineDevs, nsLeafDevs, ewSpineDevs, ewLeafDevs, epDevs }) => {
    ...
    const renderCompactLogicalDeviceCard = (dev, bgClass) => {
        ...
        const tStyle = LIGHT_THEME_STYLES[dev.theme] || LIGHT_THEME_STYLES.slate;
        ...
        return (
            <div
                ...
                className={`relative rounded-xl border cursor-pointer transition-all ${cardWidthClass} shrink-0 flex flex-col ${tStyle.bg} ${isSelected ? 'ring-2 ring-white z-30 brightness-125 shadow-xl' : 'hover:brightness-110 z-20 shadow-md'} ${isSearchMatch ? 'ring-2 ring-yellow-400 animate-pulse border-yellow-400 shadow-[0_0_12px_#facc15] z-30' : 'border-slate-200'} overflow-hidden group mx-auto`}
            >
                {/* Header Area */}
                <div className={`p-2 flex items-center justify-center gap-2 relative bg-black/20 border-b border-slate-200`}>
                    ...
                    <div className="min-w-0 flex flex-col items-center">
                        <div className="text-sm font-bold text-slate-900 truncate">{dev.customName}</div>
                        <div className="text-[10px] text-slate-500 font-mono truncate flex items-center gap-1.5 justify-center">
                            <span>[{racks.find(r => r.id === dev.rackId)?.name}]</span>
                            {portLayout && (
                                <span className="px-1 py-0.5 rounded bg-slate-800 text-[9px] text-slate-700 font-semibold">
                ...
                {/* Ports Area */}
                <div className="p-3 flex flex-col gap-2 bg-slate-100/40">
                    {portLayout ? (
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center justify-center gap-2">
                                ...
                                {(() => {
                                    ...
                                    return (
                                        <div className="flex border-l border-slate-200 pl-2">
                        ...
                        return (
                            <div className={is2U4N ? "grid grid-cols-2 gap-x-4 gap-y-2 w-full px-2" : "flex flex-col gap-2 w-full px-2"}>
                                {nodes.map((nodeKey, idx) => {
                                    ...
                                    let cellClass = "";
                                    if (is2U4N) {
                                        if (idx === 0) cellClass = "border-b border-r border-slate-200 pb-2 pr-4";
                                        else if (idx === 1) cellClass = "border-b border-slate-200 pb-2 pl-4";
                                        else if (idx === 2) cellClass = "border-r border-slate-200 pt-2 pr-4";
                                        else if (idx === 3) cellClass = "pt-2 pl-4";
                                    } else {
                                        cellClass = !isLast ? 'border-b border-slate-200 pb-2' : 'pt-1';
                                    }

                                    return (
                                        <div key={nodeKey} className={`flex flex-wrap items-center justify-end gap-3 ${cellClass}`}>
                                            <div className="text-[11px] font-bold text-slate-500 mr-auto flex items-center gap-1">
                                            ...
                                            {Array.from({ length: pcieSlotQty }).map((_, i) => {
                                                ...
                                                return (
                                                    <div key={slotIdx} className="flex items-center gap-1.5">
                                                        <div className="text-[10px] font-bold font-mono text-slate-500">{model}</div>
                            ...
                            <div className="flex flex-col gap-2">
                                {ewNicCount > 0 && projectInfo?.designType !== 'msft' && (
                                    <div className="flex justify-center">
                                        <span className="text-[10px] font-bold font-mono text-slate-500 mr-2 mt-0.5">EW NIC</span>
                                ...
                                <div className="flex flex-wrap justify-center gap-4 border-t border-slate-200 pt-2 mt-1">
                                    {Array.from({ length: pcieSlotQty }).map((_, i) => {
                                        const slotIdx = i + 1;
                                    ...
                                    {ocpCount > 0 && <div className="flex items-center gap-1.5"><div className="text-[10px] font-bold font-mono text-slate-500">{dev.hardwareSpecs?.ocp?.model || 'OCP'}</div><div className="flex gap-1.5">{Array.from({ length: ocpCount }).map((_, i) => renderPortAnchor(dev, `ocp-${i + 1}`, `OCP P${i + 1}`, 'hover:border-amber-400 hover:bg-amber-500/50'))}</div></div>}
                                    {superNicMgtCount > 0 && (
                                        <div className="flex items-center gap-1.5">
                        ...
                        const renderCustomNetBlock = () => (
                            <div className={`flex items-center gap-1.5 ${isLeft ? 'border-r pr-3' : 'border-l pl-3'} border-slate-200`}>
                                <div className="text-[10px] font-bold font-mono text-cyan-600 leading-normal pb-0.5">{customNet.name || 'group1'}</div>
    ...
    const renderTreeSection = (devList, bgClass, labelPrefix, isRow = false) => {
        ...
        if (groups.length === 0) return <div className="text-slate-500 text-sm py-4 italic border border-dashed border-slate-200 rounded-lg w-full text-center">無設備</div>;
        ...
        return (
            <div className={`flex ${isRow ? 'flex-nowrap justify-center min-w-max' : 'flex-wrap justify-center'} gap-8 w-full z-10 p-4 pb-6`}>
                {groups.map(group => {
                        ...
                        return (
                            ...
                            >
                                <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Maximize2 className="w-5 h-5 text-slate-500 hover:text-white" />
                                </div>
                                <div className="text-2xl font-bold text-slate-900 tracking-wider text-center">{group.name}</div>
                                <div className="text-sm text-slate-500 mt-2 font-mono">{group.devs.length} Devices</div>
                    ...
                    return (
                        <div key={group.name} className={`flex flex-col items-center gap-4 bg-slate-100/40 p-4 rounded-xl border border-slate-200 ${groupWidthClass}`}>
                            <button
                                onClick={(e) => { e.stopPropagation(); setExpandedNetGroups({ ...expandedNetGroups, [`${labelPrefix}-${group.name}`]: !isOpen }); }}
                                className="flex items-center gap-1.5 text-[16px] font-bold text-slate-500 tracking-wider bg-slate-800 px-3 py-1.5 rounded hover:bg-slate-700 transition-colors w-full justify-center shadow-sm"
                            >
    ...
    return (
        <div className="flex flex-col gap-12 w-full items-center pt-8 pb-32 z-10 min-w-max">
            <div className="flex flex-row items-stretch justify-center gap-8 w-full px-8 min-w-max">
                <div className="flex-1 flex flex-col items-center gap-6 bg-white/80 p-8 rounded-3xl border border-slate-200 shadow-2xl min-w-max relative overflow-hidden">
                    ...
                    <div className="border-b border-slate-200 pb-2 w-full flex items-center relative justify-center">
                        <div className="text-[20px] font-bold text-slate-500 tracking-widest">NORTH-SOUTH FABRIC</div>
                        <button 
                            onClick={() => toggleSection(['NS-Spine', 'NS-Leaf'])} 
                            className="absolute right-0 text-sm flex items-center gap-1.5 text-slate-500 hover:text-white bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-lg transition-colors border border-slate-600"
                        >
                ...
                {projectInfo?.designType !== 'msft' && (
                    <div className="flex-1 flex flex-col items-center gap-8 bg-white/80 p-8 rounded-3xl border border-slate-200 shadow-2xl min-w-max relative overflow-hidden">
                        ...
                        <div className="border-b border-slate-200 pb-2 w-full flex items-center relative justify-center">
                            <div className="text-[20px] font-bold text-slate-500 tracking-widest">EAST-WEST FABRIC</div>
                            <button 
            ...
            <div className="flex flex-col items-center w-full gap-6 px-8 mt-6 bg-white/60 py-8 rounded-3xl border border-slate-200 shadow-2xl relative overflow-hidden min-w-max">
                ...
                <div className="border-b border-slate-200 pb-2 mb-2 w-full max-w-2xl flex items-center relative justify-center">
                        <div className="text-[20px] font-bold text-slate-500 tracking-widest">ENDPOINT LAYER</div>
                        <button 
...
export default NetworkTopologyLight;
